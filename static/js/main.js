...
function Init(feedType, feedPath, bgArr, senArr) {
    ...
    feed.addEventListener("load", function () {
        ...
        if (feed.status !== 200) {
            setArtTip('<i class="fa fa-warning"></i><p class="article-empty-tip">此功能并没有启用</p>');
            return;
        }
        var data = [];
        try {
        ...
        data.length === 0 ? setArtTip('<i class="fa fa-inbox"></i><p class="article-empty-tip">此功能并没有启用</p>') : articleOutput(data);
